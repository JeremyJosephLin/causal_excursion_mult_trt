...
SD <- readRDS("simulation/5. WA-d violated /sim 7.1a/Simulation_design.RDS")
...
for (i in setting_start:setting_end) {
  ...
  ## taut_t
  taut_t <-
    construct_taut_theta(m , SD$AvgTau_t[i], SD$tau_t_shape[i], theta = SD$tau_t_theta[i])
...
outName <- paste("results_sim_7_1a_setting_",isetting,"_",nsim,".RDS",sep = "")
