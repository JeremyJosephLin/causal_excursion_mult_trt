...
SD <- readRDS("simulation/2. WA-a violated/sim 2.5a/Simulation_design.RDS")
...
for (i in setting_start:setting_end) {
  ...
  ## taut_t
  taut_t <-
    construct_taut_theta(m , SD$AvgTau_t[i], SD$tau_t_shape[i], SD$tau_t_theta[i])
  ...
  ## taut_w
  taut_w <-
    construct_taut_theta(m , SD$AvgTau_w[i], SD$tau_w_shape[i], SD$tau_w_theta[i])
...
outName <- paste("results_sim_2_5a_setting_",isetting,"_",nsim,".RDS",sep = "")
