...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  ft_w_theta = seq(from = -1, to = 1, by = 0.01),
                  ft_w_theta2 = 0,
                  gt_w_shape = NA,
                  gt_w_theta = NA,
                  beta1_w0 = NA,
...
for (i in 1:nrow(SD)) {
  ...
  SD$p_w[i] <- ncol(ft_w)
  # trt 1 eff
  beta1_w <- solve_beta(shape = SD$ft_w_shape[i], m = m, ATE = SD$ATE1_w[i], ft = ft_w,
                        taut = taut_w, theta = SD$ft_w_theta[i])
...
summary(SD)
dim(SD)
saveRDS(SD, file = "simulation/2. WA-a violated/sim 2.5a/Simulation_design.RDS")
