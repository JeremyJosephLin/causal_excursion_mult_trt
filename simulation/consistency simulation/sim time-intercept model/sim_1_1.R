...
dgm_1 <- function(sample_size, total_T) {
  ...
  alpha_1 <- 1 #time effect on control
  
  # intercept
  beta_1 <- 0.1 # when At = 1
  beta_4 <- 0.45  # when At = 2
  
  # Moderator effect 
  beta_2 <- 0.3 # when At = 1
  beta_5 <- 0.1 # when At = 2
  
  # time effect 
  beta_3 <- 0.01 # when At = 1
  beta_6 <- 0.02 # when At = 2
  ...
  for (t in 1:total_T) {
    ...
    dta$Y[row_index] <- (dta$A1[row_index] * (beta_1 + beta_2 * dta$S[row_index] + beta_3 * dta$time[row_index])  +
                           dta$A2[row_index] * (beta_4 + beta_5 * dta$S[row_index] + beta_6 * dta$time[row_index]) +
                           dta$Y_A0[row_index]+ alpha_1 * dta$time[row_index] +
                           dta$eps[row_index])
  }
...
# utility function for estimator function 
...
)
{
  ...
  end_time <- max(dta$time)
  start_time <- min((dta$time))
  total_T <- end_time - start_time + 1
  ...
  if (is.null(pmatrix_tilde)) {
    pmatrix_tilde <- matrix(rep(round(1/nA, 2), nA* total_T), ncol = nA)
  } 
  ...
  for (i in 1:total_T) {
    ...
    temp <- dta[which(dta$time == time), treatment_varname]
    p_t_tilde[which(dta$time == time)] = match_p_tilde(temp, pmatrix_tilde[i,])
    ind <- build_W_mat(temp,pmatrix_tilde[i,] )
    ind_matrix[which(dta$time == time),] = ind$ind_matrix
    ind_center[which(dta$time == time),] = ind$ind_center
    time = time + 1 
  }
...
moderator_vars <- c("S", "time")
avail_varname = "I"

dta <- dgm_1(sample_size = 20, total_T = 20)
...
# save result list 
saveRDS(object = result_list, file = "~/Documents/Research/causal_excursion_mult_trt/simulation_results/Consistency Simulation/sim_1_1_1/sim.rds")
