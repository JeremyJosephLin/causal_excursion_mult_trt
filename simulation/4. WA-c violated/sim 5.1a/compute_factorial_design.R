# sim 5.1a : misspecification of constant variance
rm(list = ls())
...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  # m = c(30, 60),
                  m = c(30),
                  pt_shape = c("constant equal pk","constant p1 high","constant p2 high"),
                  ...
                  n = NA,
                  p_star = NA,
                  theta_jt = round(seq(- 0.8, 0.8, by= 0.1),digits = 1),
                  theta_ht = round(seq(- 0.8, 0.8, by= 0.1),digits = 1),
...
summary(SD)
dim(SD)
saveRDS(SD, file = "simulation/4. WA-c violated/sim 5.1a/Simulation_design.RDS")