...
SD <- readRDS("simulation/7. WA-f violated/sim 8.1a/Simulation_design.RDS")
...
for (i in setting_start:setting_end) {
  ...
  #SPNC <- compute_SPNC_SConY(gt_t, alpha_t, taut_t, SD$coef_SC)
  
  ## gt_w, alpha_w
  gt_w <- gt_t # include intercept to satisfy the span requirement
  ## gt_w, alpha_w
  #gt_w <-  cbind(rep(1, m), SPNC)
  #alpha_w <- c(0, 1)
...
outName <- paste("results_sim_8_1a_setting_",isetting,"_",nsim,".RDS",sep = "")
