...
```{r}
source("~/Documents/Research/causal_excursion_mult_trt/simulation_results/Power Simulation/viol_3/sim_5_1/functions_util.R")
# read simulation design 
SD <- readRDS("~/Documents/Research/causal_excursion_mult_trt/simulation_results/Power Simulation/viol_3/sim_5_1/Simulation_design.RDS")
n_seed = 6 
nsim = 500
truth_hypothesis = "H1"
dim(SD)
...
for (isim in 1:n_seed) {
  fileName = paste("results_sim_5_1a_setting_",isim,"_",nsim,".RDS",sep = "")
...
# 3. Make plots and tables ------------------------------------------------
    ...
    xlab(TeX(r'($\theta_2^w$)')) +
    ylab("power") +
    ggtitle(TeX(r'($MEE_k^w(t)$: Linear; $\theta_2^* = 0)')) +
...
ggsave(paste0("figure 4 - power.pdf"), width = 5.5, height = 8)
```
...
result %>% 
    filter(gt_shape_and_gt_t_theta != "quadratic_theta0") %>%
    filter(theta_ht == 0) %>% 
    ggplot(aes(x = theta_jt, y = power_adj, linetype = factor(factor(pt_shape)),
               color = factor(ATE_t_and_ASPNC_t))) +
    geom_line()
```
```{r}
