...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  n = NA,
                  p_star = NA,
                  ATE_tw_ratio = seq(from = 0.8, to = 1.2, by = 0.025),
...
SD$ATE1_w <- SD$ATE1_t / SD$ATE_tw_ratio
SD$ATE2_w <- SD$ATE2_t / SD$ATE_tw_ratio
...
saveRDS(SD, file = "simulation/2. WA-a violated/sim 3.1a/Simulation_design.RDS")
