...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  q_w = NA,
                  n = NA,
                  ATE_tw_diff = seq(from = -1, to = 1, by= 0.1),
                  p_star = NA,
...
SD$ATE1_w <- SD$ATE1_t + SD$ATE_tw_diff
SD$ATE2_w <- SD$ATE2_t + SD$ATE_tw_diff
...
saveRDS(SD, file = "simulation/2. WA-a violated/sim 2.2a/Simulation_design.RDS")
