...
```{r}
source("~/Documents/Research/causal_excursion_mult_trt/simulation_results/Power Simulation/viol_5/sim_6_1a/functions_util.R")
# read simulation design 
SD <- readRDS("~/Documents/Research/causal_excursion_mult_trt/simulation_results/Power Simulation/viol_5/sim_6_1a/Simulation_design.RDS")
n_seed = 6 
nsim = 500
truth_hypothesis = "H1"
dim(SD)
...
for (isim in 1:n_seed) {
  fileName = paste("results_sim_6_1a_setting_",isim,"_",nsim,".RDS",sep = "")
...
# 3. Make plots and tables ------------------------------------------------
    ...
    result$ATE_t_and_ASPNC_t <- paste0(result$delta_ATE, " ", result$ft_t_theta, " ",result$ASPNC_t)
    ...
    p2 <- result %>% 
    filter(gt_shape_and_gt_t_theta != "quadratic_theta0" ,gt_shape_and_gt_t_theta != "constant0" ) %>%
    ggplot(aes(x = coef_SC, y = power_adj, linetype = factor(ATE_t_and_ASPNC_t),
               color = factor(gt_shape_and_gt_t_theta))) +
    geom_line(size = 1)+
      geom_hline(yintercept = 0.8, linetype = 2) +
    scale_linetype_discrete(name = TeX(r'($sMEE_k^w(t)$ = $sMEE_k^*(t)$ :constant)'),
                            labels =c(TeX(r'($\Delta ATE^* = \Delta ATE^w = 0.1$)'),
                                      TeX(r'($\Delta ATE^* = \Delta ATE^w = 0.2$)'))) +
    scale_color_discrete(name = TeX(r'(Pattern of $EO^*(t) = EO^w(t))'),
    ...
    xlab(TeX(r'($\nu_1)')) +
    ylab("power") +
    #ggtitle(TeX(r'($MEE_k^w(t)$ = $MEE_k^*(t)$ :constant)')) +
...
ggsave(paste0("figure 6_1 - power.pdf"), width = 12, height = 8)
```
...
result %>% 
    ...
    filter(theta_jt == 0) %>% 
    ggplot(aes(x = theta_ht, y = power_adj, linetype = factor(factor(pt_shape)),
