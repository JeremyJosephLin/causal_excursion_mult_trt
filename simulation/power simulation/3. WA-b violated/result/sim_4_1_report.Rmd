...
```{r}
source("~/Documents/Research/causal_excursion_mult_trt/simulation_results/Power Simulation/viol_2/sim_4_1a/functions_util.R")
# read simulation design 
SD <- readRDS("~/Documents/Research/causal_excursion_mult_trt/simulation_results/Power Simulation/viol_2/sim_4_1a/Simulation_design.RDS")
n_seed = 6 
nsim = 500
truth_hypothesis = "H1"
dim(SD)
...
for (isim in 1:n_seed) {
  fileName = paste("results_sim_4_1a_setting_",isim,"_",nsim,".RDS",sep = "")
...
result %>% 
    ...
    filter(! ATE_t_and_ASPNC_t %in% c("0.1 0 linear_theta","0.1 0 linear_theta")) %>% 
    ggplot(aes(x = ASPNC_tw_ratio, y = power_adj, linetype = factor(ATE_t_and_ASPNC_t),
               color = factor(gt_t_shape_and_theta))) +
    facet_grid(~ ASPNC_name, labeller = label_parsed)+
    geom_line()+
    geom_hline(yintercept = 0.8, linetype = 2) +
    scale_linetype_discrete(name = TeX(r'(Pattern of $sMEE^*(t) = sMEE^w(t)$)'),
                         labels = c(TeX(r'($\Delta ATE = 0.3$ linear, $\theta_f^* = \theta_f^w = -0.3$)'),
                                    ...
                                    TeX(r'($\Delta ATE = 0.4$ linear, $\theta_f^* = \theta_f^w = 0.3$)'))) +
    scale_color_discrete(name = TeX(r'(Pattern of $EO^*(t)$ and $EO^w(t)$ )'),
                            labels = c(
...
ggsave(paste0("figure 4_1 - power.pdf"), width = 12, height = 8)
```
