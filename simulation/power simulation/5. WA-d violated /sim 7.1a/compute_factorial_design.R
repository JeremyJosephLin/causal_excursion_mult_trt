# sim 7.1a : misspecification of availability working const 
rm(list = ls())
...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  AvgTau_t = c(0.6),
                  tau_t_shape = c("linear", "sine"),
                  tau_t_theta = seq(from = - 0.3, to = 0.3, by = 0.05),
                  beta1_t0 = NA,
                  beta1_t1 = NA,
                  beta1_t2 = NA,
                  beta2_t0 = NA,
...
summary(SD)
dim(SD)
saveRDS(SD, file = "simulation/5. WA-d violated /sim 7.1a/Simulation_design.RDS")
