...
```{r}
source("~/Research/causal_excursion_mult_trt/simulation_results/Power Simulation/viol_4/sim_7_2/functions_util.R")
# read simulation design 
SD <- readRDS("~/Research/causal_excursion_mult_trt/simulation_results/Power Simulation/viol_4/sim_7_2/Simulation_design.RDS")
n_seed = 5 
nsim = 500
truth_hypothesis = "H1"
dim(SD)
...
for (isim in 1:n_seed) {
  fileName = paste("results_sim_7_2a_setting_",isim,"_",nsim,".RDS",sep = "")
...
for (isetting in 1:setting) {
  ...
  saveRDS(result_df_collected, file = paste0("result_df_7_2collected_nsim", nsim, ".RDS"))
  saveRDS(result_list_collected, file = paste0("result_list_7_2collected_nsim", nsim, ".RDS"))
  write.csv(result_df_collected, file = paste0("result_df_7_2collected_nsim", nsim, ".csv"))
...
# 3. Make plots and tables ------------------------------------------------
    ...
    result <- readRDS(paste0("result_df_7_2collected_nsim", nsim, ".RDS"))
...
result <- result %>% filter(! tau_t_shape_and_tau_t_theta %in% c("linear0", "sine0", "constant-0.2", "constant0.2"))
...
ggsave(paste0("figure 7_2 - power.pdf"), width = 18, height = 6)
```


```{r}
result %>% 
  filter(tau_t_shape == "constant", ATE_t_and_ASPNC_t == "0.4 -0.3 linear_theta",
         gt_shape_and_gt_t_theta == "quadratic_theta-0.3") %>% 
    ggplot(aes(x = AvgTau_tw_ratio, y = power_adj, linetype = factor(gt_shape_and_gt_t_theta),
               color = factor(ATE_t_and_ASPNC_t))) +
    geom_line()
```
