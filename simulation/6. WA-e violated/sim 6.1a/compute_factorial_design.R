# sim 6.1a : serial correlation misspecification 
...
for (i in 1:nrow(SD)) {
  ...
  SPNC <- compute_SPNC_SConY(gt_t, alpha_t, tau_t, coef_SC)
  ...
  SD$q_w[i] <- ncol(gt_w)
  alpha_w <- solve_alpha(SD$gt_w_shape[i], m, SD$ASPNC_t_empirical[i], gt_w, taut_w,
...
summary(SD)
dim(SD)
saveRDS(SD, file = "simulation/6. WA-e violated/sim 6.1a/Simulation_design.RDS")
