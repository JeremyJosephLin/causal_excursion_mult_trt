# sim 8.1a : endogeneous availabilty
...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  # m = c(30, 60),
                  m = c(30),
                  gamma3 = seq(-0.2, 0.2, by = 0.01),
                  gamma4 = seq(-0.2, 0.2, by = 0.01),
                  pt_shape = c("constant equal pk"),
                  # pt_shape = c("constant 0.5"),
                  #coef_SC = seq(-1, 1, by= 0.1),
                  ...
                  #tau_t_theta = seq(from = - 0.4, to = 0.4, by = 0.1),
                  beta1_t0 = NA,
                  beta1_t1 = NA,
                  beta1_t2 = NA,
                  beta2_t0 = NA,
...
for (i in 1:nrow(SD)) {
  ...
  if (!is.na(SD$n[i])) {
    ...
    dta <- dgm_endoAvail(SD$n[i], total_T = m, ft = ft_t, beta_1 = beta1_t, beta_2 = beta2_t, gt = gt_t, alpha = alpha_t, tau = taut_t, pt = pt, 
...
summary(SD)
dim(SD)
saveRDS(SD, file = "simulation/7. WA-f violated/sim 8.1a/Simulation_design.RDS")
