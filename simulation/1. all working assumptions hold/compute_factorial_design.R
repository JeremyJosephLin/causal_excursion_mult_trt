...
# dgm will be able to 
rm(list = ls())
# setwd("~/Documents/Research/causal_excursion_mult_trt/github_repo/causal_excursion_mult_trt/simulation/all working assumptions hold/")
source("")
source("~/Documents/Research/causal_excursion_mult_trt/github_repo/causal_excursion_mult_trt/functions/")  
...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  ft_t_shape = c("linear_theta", "constant"),
...
for (i in 1:nrow(SD)) {
  ...
  # trt 2 effect
  beta2_t <- solve_beta(SD$ft_t_shape[i], m, SD$ATE2_t[i], ft_t, taut_t, theta = SD$ft_t_theta[i])
  ...
  # trt 2 eff
  beta2_w <- solve_beta(SD$ft_w_shape[i], m, SD$ATE2_w[i], ft_w,
                        taut_w, theta = SD$ft_w_theta[i])
...
summary(SD)
saveRDS(SD, file = "Simulation_design.RDS")
