...
# dgm will be able to 
rm(list = ls())
setwd("~/Documents/Research/causal_excursion_mult_trt/Data Analysis/HeartStep/Sample Size calculation /")
...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  ATE1_t = seq(from = 0.08, to = 0.25, by = 0.01),
                  ATE1_t_empirical = NA,
                  ATE2_t = c(0.00005),
                  ATE2_t_empirical = NA,
                  ASPNC_t = c(3.0),
...
for (i in 1:nrow(SD)) {
  ...
  tryCatch({
    ...
    SD$n[i] <- mrt_mult_trt_ss(ft_w, gt_w, beta_w, alpha_w, pt, mean_var = mean_var, gamma, b,
                               L, tau = taut_w)
    M_temp <- compute_sigma(f_t = ft_w, beta = beta_w, p_t = pt, L, nA = 3, tau = taut_w)
    var_beta <- solve(M_temp$M_matrix)
    SD$se_beta1_w0[i] <- sqrt(var_beta[1,1]* 1/SD$n[i]) 
    SD$se_beta2_w0[i] <- sqrt(var_beta[2,2]* 1/SD$n[i]) 
    SD$p_star[i] <- dim(L)[1]
  },
  error = function(cond){
...
SD <- expand.grid(gamma = 0.05,  # type I error
                  b = 0.2, # type II error
                  # m = c(30, 60),
                  m = seq(from = 30, to = 210, by =10),
                  pt_shape = c("constant equal pk"),
                  # pt_shape = c("constant 0.5"),
                  ft_t_shape = c("constant", "linear_theta"),
                  ft_t_theta = c(0, 0.4),
                  ft_t_theta2 = c(0, 0.2, 0.6, 0.9),
...
summary(SD)
dim(SD)
saveRDS(SD, file = "n_vs_m.RDS")

# n vs m with different ASPNC for large delta ATE

SD <- expand.grid(gamma = 0.05,  # type I error
                  b = 0.2, # type II error
                  # m = c(30, 60),
                  m = seq(from = 30, to = 300, by =10),
                  pt_shape = c("constant equal pk"),
                  # pt_shape = c("constant 0.5"),
                  ft_t_shape = c("constant"),
                  ft_t_theta = c(0) ,
...
summary(SD)
dim(SD)
saveRDS(SD, file = "n_vs_m_EO_small_n.RDS")

# n vs m with different ASPNC for small delta ATE
...
summary(SD)
dim(SD)
saveRDS(SD, file = "n_vs_m_EO_large_n.RDS")
...
SD <- expand.grid(gamma = 0.05,  # type I error
                  ...
                  # for specifying gt when gt_t_shape is linear or quadratic
                  # ATE_t = c(1.2, 1.3, 1.4, 1.5),
                  ATE1_t = seq(from = .08, to = 0.18, by = 0.01),
                  ATE1_t_empirical = NA,
                  ATE2_t = c(0.00005),
                  ATE2_t_empirical = NA,
                  ASPNC_t = seq(from = 2.5, to = 3.3, by = 0.01),
                  ASPNC_t_empirical = NA,
                  #AvgTau_t = c(0.5, 0.8, 1),
                  AvgTau_t = c(1),
                  tau_t_shape = c("constant"),
...
summary(SD)
dim(SD)
saveRDS(SD, file = "ATE_vs_AEO.RDS")
